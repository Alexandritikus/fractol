...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/02/17 11:12:54 by oshudria          #+#    #+#             */
/*   Updated: 2017/02/23 15:37:00 by oshudria         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

static void	fill_burning_ship(t_win *env, t_fract_param s)
{
	int		k;
	int		color;
	double	cardio;
	...
	{
		while (++k < env->f->max_iter && (s.zx * s.zx + s.zy * s.zy) < 10)
		{
			s.tz = s.zx * s.zx - s.zy * s.zy;
			s.zy = 2 * fabs(s.zx * s.zy);
			s.zx = s.tz;
			s.zx = s.zx - s.cx;
			s.zy = s.zy - s.cy;
		}
